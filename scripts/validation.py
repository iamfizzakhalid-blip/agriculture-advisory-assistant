...
def validate_query_pre_rag(user_query: str, english_query: str) -> dict:
    ...
    system_prompt = (
        ...
        "- is_agriculture_related (boolean): true if the query is about agriculture, "
        "farming, crops, livestock, or related topics. Also true for general chatbot "
        "greetings or conversational interactions.\n"
        "- query_for_rag (string): the best English query to send to the retrieval system. "
        "If the translation is correct, use it. If the translation is wrong or misleading, "
        "provide a corrected English version based on the ORIGINAL query's actual meaning.\n"
        "- reason (string): a brief explanation of your assessment.\n\n"
        "IMPORTANT:\n"
        "- Do NOT blindly trust the translation. If the original and English versions "
        "do not match, prefer the original query's true meaning.\n"
        "- If there is suspicious or mistranslated wording that could cause incorrect "
        "retrieval, flag it and provide a safe corrected query.\n"
...
def validate_answer_post_rag(
    ...
    system_prompt = (
        ...
        "vocabulary when the user is using Urdu or Roman Urdu. Check for words like: "
